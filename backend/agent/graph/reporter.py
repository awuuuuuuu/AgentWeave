...
- **MCP 数据优先原则**：只要 MCP 工具返回了某项实时数据，就必须用该数据作为 value，哪怕知识库文档中没有对应规程；绝不能因为知识库没提及而标注为"未知"。知识库中的"储备标准/规程要求"仅作对比参考，不得将其误作"当前库存"填入 value。
- 所有 MCP 返回的物资库存条目都必须出现在指标列表中，不得遗漏。"""
